            ...
            <nav className={`py-4 lg:px-24 px-4  ${isSticky ? "sticky top-0 right-0 left-0 bg-blue-300" : ""}`}>
                <div className='flex justify-between items-center text-base gap-8'>
                    ...
                    <ul className='md:flex space-x-12 hidden'>
                        ...
                        {user ? <Logout /> : <Link to="/sign-up" className='block text-base text-black uppercase cursor-pointer hover:text-blue-700'>Signup</Link>}
                    </ul>
                    ...
                    <div className={`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}`}>
                        ...
                        {user && <Logout />}
                        {!user && <Link to="/sign-up" className='block text-base text-white uppercase cursor-pointer'>Signup</Link>}
                    </div>
                </div>
